...
export RUSTFS_OBS_ENDPOINT=http://localhost:4317 # OpenTelemetry Collector endpoint
#export RUSTFS_OBS_USE_STDOUT=false # Whether to use stdout output
#export RUSTFS_OBS_SAMPLE_RATIO=2.0 # Sampling ratio, 0.0-1.0, 0.0 means no sampling, 1.0 means full sampling
...
# notify
export RUSTFS_NOTIFY_WEBHOOK_ENABLE="true" # Whether to enable webhook notifications
export RUSTFS_NOTIFY_WEBHOOK_ENDPOINT="http://[::]:3020/webhook" # Webhook notification endpoint
export RUSTFS_NOTIFY_WEBHOOK_QUEUE_DIR="$current_dir/deploy/logs/notify"


export RUSTFS_NS_SCANNER_INTERVAL=60  # Object scanning interval time in seconds
...
# Start main service
cargo run --release --bin rustfs -- --address ${RUSTFS_ADDRESS} ${RUSTFS_VOLUMES}
